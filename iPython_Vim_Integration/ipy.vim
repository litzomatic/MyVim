...
if findstart:
    line = str(vim.eval("getline('.')"))
    cursor = int(vim.eval("col('.') - 1"))

    base_word_length = 0
    reverse_sub_line = line[0:cursor][::-1]

    is_file_completion = re.compile(r'.*?/[^ \t\'"]')
    if not is_file_completion.search(reverse_sub_line):
        #find_base_word = re.compile(r'(.*?)(?! \\)(?:[ \t\'"]|$)')
        find_base_word = re.compile(r'(.*?)(?:[ \t\'"\(\[\{]|$)')
#\(\[\{
        match = find_base_word.match(reverse_sub_line)

        if match == None:
            base_word_length = 0
        else:
            base_word_length = len(match.group(1))
    else:
        """This version of find_base_word just returns grouping of text
        between slashes as well as the start and end slash"""
        find_file_path_base = re.compile(r'(.*?/(?:.*/)?\.?)')
        match = find_file_path_base.match(reverse_sub_line)
    ...
    #for letter in reverse_sub_line:
        #if letter == ' ' or letter == '\t':
            #break
        #
    ...
    start = cursor - base_word_length

    #TODO use logging
    with open('/tmp/ipython-dev.txt', 'ab') as f:
        f.write('Find Start\n')
        f.write('Base: %s\n' % str(base))
        f.write('findstart: %s\n' % str(findstart))
        f.write('currentline: %s\n' % str(vim.current.line))
        f.write('cursor: %s\n' % str(vim.eval("col('.')")))
        f.write('start: %s\n' % str(start))
        f.write('sub-line: %s\n' % line[0:cursor])
        f.write('calc-base: %s\n' % line[start:cursor])
    ...
    vim.command("let l:start = %s" % start)

else:
    with open('/tmp/ipython-dev.txt', 'ab') as f:
        f.write('Find Matches\n')
     
    current_line = vim.current.line

    if re.match(r'^/.*/$', base):
        #TODO define a function for next two lines
        base = re.sub(r' ', r'\ ', base)
        current = re.sub(r' ', r'\ ', base)

    msg_id = km.shell_channel.complete(base, current_line, vim.eval("col('.')"))

    tries = 0
    while True:
        """I'm pretty sure this was the source of random exceptions I'm
        seeing, making it better"""
        try:
            time.sleep(.1)
    ...
    #TODO using loggin
    with open('/tmp/ipython-dev.txt', 'ab') as f:
        f.write('Find Choices\n')
        f.write('Base: %s\n' % str(base))
        f.write('findstart: %s\n' % str(findstart))
        f.write('message: %s\n' % str(m))
        f.write('\n')

    matches = [re.sub(r'\\', '', str(match)) for match in m['content']['matches']]
    if len(matches) > 1:
        """When there is one match, I like it when it's used instantly, I can
        go back by typing ctrl-n but, when there are several matches I'd prefer
        the first option to be what I originally had (usually if there's a lot
        I'd prefer to type some more to narrow it down)"""
        matches.insert(0, re.sub(r'\\', '', str(m['content']['matched_text'])))

    vim.command("let l:choices = %s" % matches)
...
"m = get_child_msgs(msg_id)[0]
"
"matches = [str(match) for match in m['content']['matches']]
""
"with open('/tmp/ipython-dev.txt', 'ab') as f:
    "f.write(str(matches) + "\n")
"
...
"
"#"""Vim already supports the "no completion" version"""
"#matches.append(base) # the "no completion" version
"
"completions = matches
